...
var option = [ {"id":"jong","name":"中国麻雀"} ,{"id":"nyan","name":"にゃんにゃん～"} ]
...
export default {
  ...
  methods: {
    ...
    async getConfig() {
      ...
      var response = await axios.get('http://'+host+'/jong/config/'+this.game_type)
      console.log(response.data)
      this.detail_options = response.data;
      for(var index in this.detail_options){
        this.detail_options_selected[index] = this.detail_options[index].default
      }
    },
    async createRoom(){
...
function new_socket_conf() {
  return new Promise( (res,rej) => {
    var host = location.host
    if (location.port == 8080) { host = location.hostname + ':8000' }
    var socket = window.socket = new WebSocket('ws://' + (host) + '/jong/room/configured' )
