<template>
  <div>
    ...
    <command-bar
      v-show="main"
      :class="{'player-main':main}"
      :commands_available="player.commands_available"
      @command="command"
    />
    <table
      class="player-hand"
      :class="{'border-discard-hand':player.allow_discard,'player-main':(main||open)}"
    >
      <tr>
    ...
    <div class="bottom-bar">
      ...
      <div class="exposed-area">
        <exposed-set
          v-for="(grp,index) in player.exposed"
          v-bind:type="grp.type"
          v-bind:tiles="grp.tiles"
          v-bind:target="player.target=='apkong'"
          v-bind:show_conc="(main || open)"
          :key="index"
        ></exposed-set>
      </div>
    </div>
