...
function get_wind_name(id){
  return _wind_name[id];
}

function tile_click(x){
  var pl = deck.players[deck.player_id];
...
async function click_meld_popup(pos){
  ...
  if(input_resolve != null){
    meld_selection.meld_selection = [] ;
    input_resolve(meld_selection.type , meld_selection.pos[pos] );
  }
}
...
function command(type){
  ...
  if(fil.length == 1){
    ...
    meld_selection.type = type ;
    meld_selection.meld_selection = tiles ;
    meld_selection.pos = selections ;
  }
}
...
function timer_interval(resolve,timeout,cancelObj){
...
async function timer(timeout,cancelObj){
  var cancel = null;
